...
async def insert_incidencia_view(request):
    ...
    if resumen and descripcion and servicio and prioridad and estado and usuario and fecha_deseada:
        await insert_incidencia(resumen, descripcion, servicio, prioridad, estado, usuario, fecha_deseada)
...
# Ruta POST para actualizar o eliminar incidencia según estado
@app.post("/editar/<id>")
async def editar_incidencia_post(request, id):
    ...
    result = await update_or_delete_incidencia(
        id,
        resumen=resumen,
